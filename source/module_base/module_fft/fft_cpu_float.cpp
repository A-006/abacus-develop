...
void FFT_CPU<float>::setupFFT()
{
    ...
    if (!this->mpifft)
    {
        z_auxg = (std::complex<float>*)fftw_malloc(sizeof(fftw_complex) * this->maxgrids);
        z_auxr = (std::complex<float>*)fftw_malloc(sizeof(fftw_complex) * this->maxgrids);
        d_rspace = (float*)z_auxg;
